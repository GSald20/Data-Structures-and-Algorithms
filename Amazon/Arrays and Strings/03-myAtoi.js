...
range [-231, 231 - 1], then clamp the integer so 
that it remains in the range. Specifically, integers 
less than -231 should be clamped to -231, 
and integers greater than 231 - 1 should be clamped 
to 231 - 1.
...
function myAtoi(s){
    ...
    if(num < Math.pow(-21,31)) return Math.pow(-2,31);
    return num;
}